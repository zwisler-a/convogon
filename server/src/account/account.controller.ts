...
export class AccountController {
    ...
    @ApiResponse({status: 200, description: 'Successfully retrieved user payment status.', type: AccountStatusDto})
    ...
    @UseGuards(AdminAuthGuard)
    @ApiBearerAuth()
    @ApiOperation({summary: 'Get user by ID', description: 'Fetch a single user by their unique ID (admin only).', operationId: 'getAccountById'})
    @ApiParam({
        name: 'id',
        description: 'Unique identifier of the user',
        example: 'a3f8c9b0-1a2b-4e6d-b3b9-8f6a5c5a9e1d'
    })
    @ApiResponse({status: 200, description: 'Successfully retrieved user details.', type: AccountEntity})
    ...
    @ApiBody({
        schema: {
            properties: {
                userId: {type: 'string', example: 'a3f8c9b0-1a2b-4e6d-b3b9-8f6a5c5a9e1d'},
                shouldPay: {type: 'boolean', example: true},
                payed: {type: 'boolean', example: false},
            },
        },
    })
    @ApiResponse({status: 200, description: 'Successfully updated payment information.'})
    @ApiResponse({status: 404, description: 'User not found.'})
    @ApiResponse({status: 401, description: 'Unauthorized'})
    @Post('payment')
